...
void __dywoqc_error_fatal_message(const char *__message) { fprintf(stderr, "%s\n", __message); }

void __dywoqc_error_fatal_exit() { exit(EXIT_FAILURE); }

void __dywoqc_error_fatal(const char *__message) {
  __dywoqc_error_fatal_message(__message);
  __dywoqc_error_fatal_exit();
}

DYWOQC_NODISCARD dywoqc_error *dywoqc_error_create(const char *__message) {
...
void dywoqc_error_exit(const dywoqc_error *__err) {
  fprintf(stderr, "%s", __err->message);
  __dywoqc_error_fatal_exit();
}

void dywoqc_error_free(dywoqc_error *__err) {
